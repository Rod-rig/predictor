...
describe('PredictionStore', () => {
  ...
  const mockWithFilter = new PredictionStore({filter: 'tournament_id=sr:tournament:37'});
  ...
  it('mock with filters should have less matches', () => {
    expect(mockStore.matches.length).toBeGreaterThan(mockWithFilter.matches.length);
  });
  ...
  it('should submit form', () => {
    ...
    const submitSpy = jest.fn(mockStore.handleSubmit.bind(mockStore, new Event('submit')));
    submitSpy();
