...
module.exports.getAllTournaments = (req, res) => {
  axios.get(`${apiUrl}/en/tournaments.json?api_key=${apiKey}`)
    .then((response) => {
...
module.exports.getStandings = (req, res) => {
  axios.get(`${apiUrl}/en/tournaments/${req.params.id}/standings.json?api_key=${apiKey}`)
    .then((response) => {
      res.status(200).send(response.data);
    });
};
...
module.exports.getResults = (req, res) => {
  axios.get(`${apiUrl}/en/tournaments/${req.params.id}/results.json?api_key=${apiKey}`)
    .then((response) => {
      res.status(200).send(response.data);
    });
};
